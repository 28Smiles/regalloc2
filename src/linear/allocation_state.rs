...
use std::vec;
use crate::{Allocation, Function, Inst, MachineEnv, PReg, PRegSet, RegClass, SpillSlot, VReg};
...
pub struct AllocationState<'a, F: Function> {
    ...
    // Preffered registers
    preferred_regs_by_class: [PRegSet; 3],
    non_preferred_regs_by_class: [PRegSet; 3],
    // Spill slots
    num_spillslots: u32,
    free_spillslots_per_class: [Vec<SpillSlot>; 3],
    spillslot_allocations: Vec<VReg>,
    // PRegs
    register_state: PRegSet,
...
impl<'a, F: Function> AllocationState<'a, F> {
    ...
    pub fn new(func: &'a F, mach_env: &'a MachineEnv) -> Self {
        let mut preferred_regs_by_class = [PRegSet::empty(); 3];
        ...
        let mut non_preferred_regs_by_class = [PRegSet::empty(); 3];
        for reg_class in [RegClass::Int, RegClass::Float, RegClass::Vector] {
        ...
        let preg_allocations = vec![PRegAlloc {
            vreg: VReg::invalid(),
    ...
    #[inline]
    pub fn find_free_preg(&self, reg_class: RegClass, ignore_regs: PRegSet) -> Option<PReg> {
        ...
        if let Some(preg) = free_regs.first() {
            return Some(preg);
        }
    ...
    #[inline]
    pub fn find_furthest_used_preg(&self, reg_class: RegClass, ignore_regs: PRegSet) -> Option<PReg> {
    ...
    #[inline]
    pub fn get_preg_vreg(&self, preg: PReg) -> VReg {
